...
- **주요 구성 및 UX 규격 (모바일 스크롤 최소화 원칙)**:
  1. **초슬림 콤팩트 헤더 (Compact Header - 검색창 전면 배제)**:
     - 불필요한 수직 공간을 차지하는 통합 검색창은 완전 배제.
     - 슬림한 브랜드 배지 및 타이틀과 함께 우측 또는 하단에 `전체`, `금융`, `생활`, `통화` 카테고리 퀵 칩을 한 줄로 압축 배치하여 수직 높이를 최소화.
  2. **모바일 2열 콤팩트 카드 그리드 (Mobile 2-Column Grid)**:
     ...
     - 불필요하게 긴 텍스트 문단을 줄이고, [아이콘 + 계산기 명칭 + 1줄 핵심 기능 키워드] 중심의 정갈한 콤팩트 카드 레이아웃 적용.
     - 5대 활성 계산기(연복리, 대출, 연봉, 단위, 환율)가 모바일 1~2 스와이프 내에 모두 손쉽게 노출되어 터치 진입성 극대화.
  3. **하단 미니멀 가치 칩 (Minimal Footer Bar)**:
     - 자리를 많이 차지하는 거대한 3단 박스 배너를 제거하고, 하단에 1줄 슬림 인라인 텍스트 칩("100% 오프라인 PWA · 광고 없음 · 즉시 연산")으로 경량화.
  4. **반응형 규격**:
