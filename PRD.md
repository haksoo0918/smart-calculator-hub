...
### 2.6 스마트 계산기 허브 메인 홈 화면 명세 (Home Dashboard - 신규 기획)
- **도입 목적**:
...
- **주요 구성 및 UX 규격**:
  1. **히어로 브랜드 섹션 (Hero Section)**:
     - Ghost 미니멀 디자인 기반 브랜드 타이틀, 슬로건 및 직관적인 계산기 통합 검색 입력창.
     - 키워드(예: "평수", "대출", "연봉", "환율", "복리") 입력 시 실시간 필터링 및 해당 계산기 즉시 점프.
  2. **카테고리별 계산기 카드 그리드 (Category Cards Grid)**:
     - **금융 & 자산**: 연복리 & 자산성장, 대출이자 & 상환비교, 연봉 실수령액
     - **생활 & 편의**: 아파트 평수 & 단위 변환, 실시간 환율 계산기
     - 각 카드별 시맨틱 Lucide 라인 아이콘, 핵심 기능 3줄 요약, 바로가기 버튼 제공.
  3. **자주 찾는 마이크로 프리뷰 위젯 (Quick Preview Widgets)**:
     - "국민 평수 84㎡는 25.4평", "오늘의 달러(USD) 환율", "연봉 5,000만 원 월 실수령액" 등 홈 화면에서 바로 확인할 수 있는 미니 인터랙션 위젯.
  4. **반응형 리플로우**:
     - 모바일: 1~2열 카드 그리드 및 상단 카테고리 퀵 칩.
     - 데스크톱: 시원한 3열 반응형 그리드 및 Electric Lime 테두리 호버 인터랙션.
