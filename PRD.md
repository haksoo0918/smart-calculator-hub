...
### 14.1 목적 및 원칙
- 본 서비스는 360px 모바일 화면부터 태블릿(768px), 데스크톱(1024px, 1280px+)까지 가로 스크롤(오버플로우) 없이 단 하나의 UI 깨짐도 허용하지 않는 **반응형 견고함(Responsive Robustness)**을 필수 원칙으로 합니다.
- 텍스트 줄바꿈으로 인해 버튼이나 카드의 높이가 제각각 튀거나, 드롭다운 셀렉트와 액션 버튼이 겹치거나 잘리는 현상을 원천 방지합니다.
