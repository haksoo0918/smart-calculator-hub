...
- **주요 구성 및 UX 규격**:
  1. **히어로 브랜드 & 검색 섹션 (Hero Section)**:
     - Ghost 미니멀 디자인 기반 브랜드 타이틀: *"일상과 금융을 위한 스마트 계산기 허브"*
     - 보조 슬로건: *"복잡한 계산은 스마트하게, 설치 없이 즉시 실행되는 오프라인 PWA 멀티 계산기"*
     - **실시간 스마트 통합 검색창 (`SearchInput`)**:
       - 검색어 입력창 (플레이스홀더: *"계산기 이름이나 키워드 검색 (예: 대출, 연봉, 평수, 환율, 복리)"*)
       - 한글 키워드 실시간 매칭(이름, 설명, 태그) 및 검색어 입력 시 즉각 카드 필터링.
       - 검색어 초기화(X) 원클릭 버튼 제공.
  2. **카테고리 퀵 필터 칩 (Category Filter Chips)**:
     - `전체 보기`, `금융 & 자산`, `생활 & 측정`, `통화 & 글로벌` 탭 칩 제공.
     - 활성 칩에는 Ghost Dark 테마의 Electric Lime(`#d1ff19`) 액센트 또는 볼드 하이라이트 적용.
  3. **계산기 카드 그리드 (Calculator Cards Grid)**:
     - **5대 활성 계산기 카드**:
       1. **연복리 & 자산성장 계산기** (`/compound`): 적립식 복리, 세금 공제, 하락장 손실, 시나리오 A/B 비교 (금융/투자)
       2. **대출이자 & 상환방식 비교** (`/loan`): 원리금균등·원금균등·만기일시 3대 상환방식 동시 비교, 거치기간 (금융/투자)
       3. **연봉 실수령액 계산기** (`/salary`): 2026년 4대보험 & 근로소득 간이세액표 기반 월/연 실수령액 & 세부 공제 (금융/급여)
       4. **단위 변환기** (`/unit`): 아파트 평(坪)↔㎡, 길이, 무게, 부피, 온도 실시간 멀티 변환 (생활/측정)
       5. **환율 계산기** (`/exchange`): 7개국 통화(USD, JPY, EUR 등) 실시간 일괄 환산 & 은행 환전 우대율 (통화/글로벌)
     - **로드맵 준비 중 카드 (Coming Soon)**:
       6. **배당금 & 월 배당 달력** (`/dividend`): 월별 현금흐름 및 세후 실수령액 (오픈 예정)
       7. **목표 자산 역산 계산기** (`/goal`): N년 뒤 목표 금액 달성에 필요한 월 적립액 역산 (오픈 예정)
     - **카드 UI 인터랙션**:
       - 전면 클릭 가능(Clickable Card) 구조로 모바일 터치 사용성 극대화.
       - Ghost 테마 카드 보더: 다크 모드 `border-slate-800 hover:border-[#d1ff19]/60`, 라이트 모드 `border-slate-200 hover:border-[#112220]`.
       - 각 카드 내부: 시그니처 Lucide 아이콘, 카테고리 태그 뱃지, 핵심 기능 요약 리스트, 하단 '계산하기' 화살표 링크.
  4. **플랫폼 핵심 가치 하이라이트 배너 (Feature Highlights)**:
     - 100% 오프라인 동작 (PWA 서비스 워커 기반)
     - 광고 없는 쾌적하고 미니멀한 UI
     - 단 하나의 Pretendard 단일 폰트와 Ghost 디자인 시스템
  5. **반응형 리플로우 (Responsive Reflow)**:
     - 모바일 (sm 미만): 1열 수직 카드 레이아웃 (카드당 넉넉한 터치 영역 확보).
     - 태블릿 (sm ~ lg): 2열 대칭 그리드.
     - 데스크톱 (lg 이상): 3열 균형 그리드.
...
- **도입 목적**:
  - 브라우저 기본 팝업 타이밍에 의존하지 않고, 사용자가 계산기 이용 중 언제든 원클릭으로 홈 화면이나 데스크톱에 앱을 설치할 수 있도록 상시 접근성 제공.
- **인터랙션 및 동작 원칙**:
  - **상시 노출 원칙**: 독립 실행형(`standalone` 모드)이 아닌 일반 브라우저 환경에서는 헤더 및 사이드바에 `[앱 설치]` 버튼을 상시 노출.
  - **적응형 스마트 분기**:
    - **네이티브 설치 지원 시 (크롬, 엣지, 안드로이드 크롬)**: `deferredPrompt.prompt()`를 즉각 실행하여 네이티브 앱 설치 대화상자 호출.
    - **미지원 또는 이벤트 대기 시 (iOS 사파리, 데스크톱 사파리, 파이어폭스 등)**: Ghost 디자인 시스템 기반의 **`PWAInstallModal` 설치 가이드 모달**을 띄워 플랫폼별 단계별 설치 방법(iOS: 하단 공유 아이콘 ➔ '홈 화면에 추가', 데스크톱: 주소창 우측 설치 아이콘 클릭 등)을 시각적으로 안내.
