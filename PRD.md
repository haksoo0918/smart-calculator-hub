...
- **주요 기능 명세**:
  1. **입력 폼 인터페이스 (`BmiForm.tsx`)**:
     - **신장(cm) 입력**: 유효범위 100cm \~ 250cm (정밀 슬라이더 + 직접 숫자 입력 필드 + 빠른 프리셋 버튼: `160cm`, `165cm`, `170cm`, `175cm`, `180cm`, 단위: 1cm).
     - **체중(kg) 입력**: 유효범위 30kg \~ 200kg (화살표 증감 단위 1kg(`step="1"`), 소수점 1자리 타이핑 지원 + 정밀 슬라이더 + 빠른 프리셋 버튼: `50kg`, `60kg`, `70kg`, `80kg`, `90kg`).
