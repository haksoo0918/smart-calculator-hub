...
### 4. 대출이자 & 상환방식 비교 계산기 (Loan Repayment Calculator)
- **3대 상환방식 동시 비교**: 원리금균등, 원금균등, 만기일시상환 방식의 총이자 및 월 상환액 나란히 비교 및 최저 이자 방식 하이라이트.
- **정밀 대출 조건 설정**: 대출 원금(한글 단위 실시간 환산), 0.1% 단위 연이율(시장 대표 금리 칩), 1~40년 대출 기간(슬라이더 지원), 거치 기간(0개월~5년 원금 유예) 지원.
- **중도상환 수수료 및 절감 이자 시뮬레이션**: 시중은행 3년(36개월) 슬라이딩 감면 공식 자동 적용 및 조기 상환에 따른 순 이익 산출.
- **Recharts 상환 추이 시각화**: 연차별 대출 잔액 감소 곡선 및 누적 납입액(원금 vs 이자) 영역 차트.
- **월별 상환 스케줄표 & CSV 다운로드**: 12개월 단위 연차별 페이지네이션, 중도상환 회차 뱃지 표기, 엑셀 호환 UTF-8 BOM CSV 내보내기 지원.
- **대출 상식 안내 카드**: DSR / DTI / LTV 개념, 상환방식 선택 가이드, 중도상환수수료 면제 시점 및 금리인하요구권 안내.
...
│   └── loan-calculator/             # 대출이자 & 상환방식 비교 계산기
│       ├── components/              # 대출 폼, 3대 상환 비교 카드, 스케줄표 등
│       └── LoanApp.tsx              # 대출 계산기 메인 컨테이너
├── components/                      # 공통 컴포넌트
