    ...
    <div className="w-full overflow-x-auto pb-1 scrollbar-none">
      ...
      >
        <TabsList
          variant="slate-solid"
          size="auto"
          className="flex items-center justify-start gap-1.5 p-1 bg-slate-100/80 dark:bg-[#1e293b] rounded-xl border border-[#e5e7eb] dark:border-slate-800 w-full min-w-max transition-colors"
        >
          {UNIT_CATEGORIES.map((cat) => {
            ...
            return (
              <TabsTrigger
                key={cat.id}
                value={cat.id}
                className="group h-auto flex items-center gap-2 px-3.5 py-2 rounded-lg text-xs sm:text-xs font-semibold transition-all duration-150 cursor-pointer text-[#475569] dark:text-slate-400 hover:bg-white/80 dark:hover:bg-slate-800 hover:text-[#112220] dark:hover:text-white data-[state=active]:bg-[#15171a] hover:data-[state=active]:bg-[#2e3238] dark:data-[state=active]:bg-slate-900 dark:hover:data-[state=active]:bg-slate-800 data-[state=active]:text-white dark:data-[state=active]:text-white data-[state=active]:shadow-xs border data-[state=active]:border-[#e5e7eb] dark:data-[state=active]:border-slate-700"
              >
