...
export const SummaryCards: React.FC<SummaryCardsProps> = ({
  ...
  const bgBadge = isIndigo ? 'bg-indigo-100 text-indigo-800' : 'bg-teal-100 text-teal-800';
    ...
    <div className="space-y-3">
      {title && (
        <div className="flex items-center gap-2">
          <span className={`text-xs font-bold px-2 py-0.5 rounded-md ${bgBadge}`}>
            {title}
          </span>
      ...
      {/* 최종 수령액 하이라이트 대형 카드 */}
      <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 to-slate-800 text-white rounded-2xl p-4 sm:p-5 shadow-sm">
